...
{
    BASEOBJECT::BaseThreadMutex *s_lua_mutex = new BASEOBJECT::BaseThreadMutex;
}
...
void lua_tinker::push(lua_State *L, char* ret, int length)
{
	lua_pushlstring(L, ret, length);
}
void lua_tinker::push(lua_State *L, const char* ret, int length)
{
	lua_pushlstring(L, ret, length);
}
...
void lua_tinker::table_obj::GetTableInfo(TableInfo& tableInfo)
{
	tableInfo.clear();
	lua_State* L = m_L;
	int         type = 0;
	...
	lua_pushnil(L);
	while (validate() && lua_next(L, m_index) != 0)
	{

		// 键值类型，如果键值是数字，表明该table是一个数组
		...
		keyValueType.push_back(keyType);
		keyValueType.push_back(valueType);
		tableInfo[key] = keyValueType;
		lua_pop(L, 1);
	}
}
