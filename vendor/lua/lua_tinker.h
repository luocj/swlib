...
{
   // init LuaTinker
    extern BASEOBJECT::BaseThreadMutex *s_lua_mutex;
   ...
   void push (lua_State *L, char* ret,int length);
   void push (lua_State *L, const char* ret, int length);
   ...
   {
       // fix: yangqy
       //BASEOBJECT::BaseGuard<BASEOBJECT::BaseThreadMutex> mutex(*s_lua_mutex);
   ...
   typedef  vector<int>					 KeyValueType;
   typedef  map<string, KeyValueType>  TableInfo;

	#define CPP_INT			1		//整形
	#define CPP_FLOAT		2		//浮点数
	#define CPP_STRING		3		//字符串
	#define CPP_BOOL		4		//bool
	#define CPP_TABLE		5		//这里指的table
   ...
   struct table_obj
   {
	  ...
	  template<typename T>
	  void set(const char* name, T object,int length)
	  {
		  if (validate())
		  {
			  lua_pushstring(m_L, name);
			  push(m_L, object,length);
			  lua_settable(m_L, m_index);
		  }
	  }
	  template<typename T>
	  ...
	  template<typename T>
	  void set(int key, T object, int length)
	  {
		  if (validate())
		  {
			  lua_pushinteger(m_L, key);
...
#define   TABLE_FOREACH_BEGIN(key,keyType,valueType,tableData)\
			{\
		   	TableInfo tableInfo;\
			tableData.GetTableInfo(tableInfo);\
			string key = "";\
			int keyType = 0;\
			int valueType = 0;\
			for (TableInfo::iterator it = tableInfo.begin(); it != tableInfo.end(); it ++)\
			{\
				key = it->first;\
				keyType = it->second[0];\
				valueType = it->second[1];

#define	 TABLE_FOREACH_END\
			}}
   

//获取值
#define GetValue(table,key,keyType,valueType)\
		(keyType) == CPP_INT ? (table).get<valueType>(atoi(key)) : (table).get<valueType>(key);
   ...
   struct table
   {
	  ...
	  void set(const char* name, T object, int length)
	  {
		  m_obj->set(name, object,length);
	  }

	  template<typename T>
	  void set(int key, T object)
	  {
		  m_obj->set(key, object);
	  }
	  template<typename T>
	  void set(int key, T object, int length)
	  {
		  m_obj->set(key, object, length);
	  }
	  ...
	  void GetTableInfo(TableInfo& tableInfo)
	  {
		  m_obj->GetTableInfo(tableInfo);
	  }

      table_obj*      m_obj;
   };

#define nillTable(L)  get<table>(L, "g_nilTable");
