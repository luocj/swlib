...
//=================End of wapped Henry Spencer's regex library ==============


//===========================================================================
//  Functions written by JoStudio
//===========================================================================

#define MAX_LENGTH  0x7FFFFFF

int RegexFind(const char *string,const char * regex_expression,int *start, int *length)
{
  regexp *r;          //regexp structure
  int i;
  int start1;         //the start point of match string
  int length1;        //the length of match string

  //parse the regex_expression
  r = regcomp(regex_expression);
  if (r == NULL) //if the regex_expression is wrong format
     return 0;

  //perform regex
  i = regexec(r, string);

  if (i==1) //if regexec retuns OK
  {
    for (i = 0; i < NSUBEXP; i++)  //scan the result in r
        if (r->startp[i] != NULL && r->endp[i] != NULL)
          {
             start1  = r->startp[i] - string;      //the match string start point
             length1 = r->endp[i] - r->startp[i];  //the match string length
             if ((length1>=0) && (length1<MAX_LENGTH))
               {
                 *start=start1;
                 *length=length1;
                 free(r);
 ...
 *  @param [in]  insert_string string to insert
 *  @param [in]  index         insert position
 *  @return  returns 0 if succeed
 *           return -1 if fail
 */
int StrInsert(char **p_string,char *insert_string, size_t index,int need_realloc)
{
  size_t length,length1;
  
  if ( (!p_string) || (!*p_string)) return -1;
  if (!insert_string) return -1;

  length=strlen(*p_string);
  length1=strlen(insert_string);
  ...
  if (index>length)
    ...
    {
      if (need_realloc==1)
         *p_string=realloc(*p_string,length+length1+1);
      memmove(*p_string+index+length1,*p_string+index,length-index+1);
      memmove(*p_string+index,insert_string,length1);
      return 0;
    }
}
/*---------------------------------------------------------------------------*/
/** Delete from string
 *  @param [in]  p_string      pointer to a string to process
 *  @param [in]  from          start position
 *  @param [in]  length         lenth of delete
 *  @return  returns 0 if succeed
 *           return -1 if fail
 */
int StrDelete(char **p_string,size_t from, size_t length,int need_realloc)
{
    size_t total_length,del_length;
    
    if ( (!p_string) || (!*p_string)) return -1;
    if (length==0) return 0;

    total_length=strlen(*p_string);
    if (from>=total_length) return -1;
    if (from+length>total_length) del_length=total_length-from; else del_length=length;

    memmove(*p_string+from,*p_string+from+del_length,total_length-del_length);
    if (need_realloc==1)
       *p_string=realloc(*p_string,total_length-del_length+1);
    (*p_string)[total_length-del_length]=0;
    return 0;
}
/*---------------------------------------------------------------------------*/
...
char * ExtractString(char *string, int start, int length)
{
   ...
   //calculator the total length of string
   total_length=strlen(string);
   if (start>=total_length) return NULL;

   //calculator the available length
   length1 = (start+length<total_length) ? length : total_length-start;

   //alloc memory for output
   result=malloc(length1+1);
   //copy from the string
   memmove(result,string+start,length1);
   result[length1]=0;
   return result;   
}
/*---------------------------------------------------------------------------*/
...
{
   //delete the match sub-string
   if (StrDelete(&string,start,length,0)==0)
     //insert the replace string
     if (StrInsert(&string,replace_string,start,0)==0)
        return 1;
     else
        return 0;
   return 0;
 ...
 */
 ...
 #define BUFSIZ  512
#endif
int RegexReplace(char *string,const char * find_regex_expr,char * replace_regex_expr,
                 int *start,int *length)
{
  ...
  int start1;         //the start point of match string
  int length1;        //the length of match string
  int replace_length; //the length of replace string
  char buf[BUFSIZ];   //a buffer to hold replace string 

  //parse the regex_expression
  r = regcomp(find_regex_expr);
  if (r == NULL) return 0; //if the regex_expression is wrong format,exit

  result = 0;
  if (regexec(r, string)==1) //if regexec retuns OK
    {
      start1=0; length1=0;
      //get the start position of matched string
      for (i = 0; i < NSUBEXP; i++)  //scan the result in r
          if (r->startp[i] != NULL && r->endp[i] != NULL)
            {
               start1  = r->startp[i] - string;      //the match string start point
      ...
      //replace string      
      if (start1>=0)  //if match string is OK
        {
           // perform regsub to get the replace string into buf
           regsub(r, replace_regex_expr, buf);
           replace_length=strlen(buf);
           //delete the match string
           if (StrDelete(&string,start1,length1,0)==0)
             {
               //insert the replace string
               if (replace_length>0)
                  if (StrInsert(&string,buf,start1,0)==0)
                    {
                       //return results
                       *start=start1;
...
/*---------------------------------------------------------------------------*/
/** Check whether char c is any char of the string
 *  @param [in]  c     char
 *  @param [in]  string   string to check
 *  @return   return ==1  if c is one of the string
 *            return ==0  if c is not one of the string
 */
int IsAnyOf(char c, char *string)
{
  size_t i;
 ...
 *  @param [in] wild_char_string   string contains wild char
...
{
   char *special_chars;  /* special char used in DOS */
   char *regex_expr;     /* regex expression for output */
   int  size;            /* size of regex expression for output */
   size_t  i; 
   
   special_chars = ".$^{[(|)+"; /* special char used in DOS */

   if (wild_char_string==NULL) return NULL;
   
   /* scan the wild_char_string and calculate the length of output regex expression */
   size=0;
   for (i=0; i<strlen(wild_char_string); i++)
     {
       size++;
       if (IsAnyOf(wild_char_string[i],special_chars))
         size++;
       else
         {
           if (wild_char_string[i]=='*') size++;
         }  
     }
   if (size==0) return NULL;

   /* alloc memory for output */
   regex_expr = malloc(size+1);
   regex_expr[size]=0;

   /* scan the wild_char_string and construct the output regex expression */
   size=0;
   for (i=0; i<strlen(wild_char_string); i++)
     {
       if (IsAnyOf(wild_char_string[i],special_chars))
         {
            regex_expr[size++]='\\';
            regex_expr[size++]=wild_char_string[i];
         }
       else
         {
           if (wild_char_string[i]=='*')
             {
               regex_expr[size++]='.';
               regex_expr[size++]=wild_char_string[i];
             }
           else if (wild_char_string[i]=='?')
             {
               regex_expr[size++]='.';
             }
           else
...
int WildCharFind(char *string, char * find_expression,int *start, int *length)
{
  ...
  regex_expr = WildCharToRegex(find_expression);
  if (regex_expr==NULL) return 0;

  result=RegexFind(string,regex_expr,start,length);
